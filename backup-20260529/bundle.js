...
Recommend manufacturing and design constraints for this product. Return JSON array only.`;
  ...
  function initAnchorPage(container) {
    ...
    function renderPhase3() {
      ...
      const dirId = _selectedDirection?.id || card.designDirection?.id;
      const matId = _selectedMaterial?.id || card.selectedMaterial?.id;
      if (!dirId || !matId) return;

      const conflicts = detectConflicts(dirId, matId);
      const confirmedCount = _constraints.filter(c => c.confirmed || c.skipped).length;
      ...
      phase3Content.innerHTML = `
          ...
          <section class="ac-section">
            <div class="ac-conflict-card">
              ...
              <div class="ac-conflict-resolutions">
                ...
                ${cf.resolutions.map(r => {
                  const resolved = _resolvedConflicts.find(rc => rc.conflictId === cf.id);
                  const isSelected = resolved && resolved.resolutionId === r.id;
                  return `
        ...
        <div class="ac-phase3-footer">
          <div class="ac-phase3-summary">
            ...
            <span>⚠ ${conflicts.length} conflict${conflicts.length !== 1 ? 's' : ''} ${conflicts.length === 0 ? '' : `(${_resolvedConflicts.length} resolved)`}</span>
          </div>
          <button class="btn btn--primary ac-complete-anchor" type="button" ${_resolvedConflicts.length < conflicts.length ? 'disabled' : ''}>
            ${_resolvedConflicts.length >= conflicts.length ? 'Complete Anchor →' : 'Resolve all conflicts to continue'}
          </button>
        </div>
      `;

      // Conflict resolution clicks
      phase3Content.addEventListener('click', (e) => {
        const btn = e.target.closest('.ac-resolution-btn');
        if (btn) {
          const cfId = btn.dataset.cf;
          const resId = btn.dataset.res;
          _resolvedConflicts = _resolvedConflicts.filter(rc => rc.conflictId !== cfId);
      ...
      // Complete anchor
      const completeBtn = phase3Content.querySelector('.ac-complete-anchor');
      if (completeBtn) {
  ...
  function initNavbar(container, { onProjectClick = null } = {}) {
    const steps = [
      { hash: '#/anchor', num: 1, label: 'Design Anchor', available: true },
      { hash: '#/compare', num: 2, label: 'Comparison', available: false },
      { hash: '#/reflect', num: 3, label: 'Reflection', available: false }
    ];
    ...
    container.innerHTML =
      `<div class="navbar__inner">
         ...
         <div class="navbar__stepper">
           ${steps.map(s =>
             `<a class="navbar__step ${s.available ? 'navbar__step--active' : 'navbar__step--disabled'}" href="${s.available ? s.hash : '#'}" data-step="${s.num}" ${!s.available ? 'data-tooltip="Coming soon"' : ''}>
    ...
    return {
      updateActiveStep(hash) {
        container.querySelectorAll('.navbar__step').forEach(el => {
          ...
          else if (!step.available) { el.classList.remove('navbar__step--active'); el.classList.add('navbar__step--disabled'); }
